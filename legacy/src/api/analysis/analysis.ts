 ...
 *
 * Stub function for opening book moves. In the full application, this
 * would fetch moves from an opening book database. For the ExpectedEval
 * MVP, this returns empty results and lets the Maia engine handle
 * all move predictions.
 *
 * @param fen - The FEN position string to look up
 * @returns A promise that resolves to an empty opening book result
 */
export const getBookMoves = async (
  _fen: string,
): Promise<{ [model: string]: { [move: string]: number } }> => {
  // MVP stub: Return empty object for all Maia models
  // In the full application, this would query an opening book API
  return {}
}
