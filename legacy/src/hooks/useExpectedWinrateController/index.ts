...
export const useExpectedWinrateController = (
  ...
  /**
   * Automatic calculation trigger when position changes
   *
   * This effect watches for position changes and automatically triggers
   * Expected Winrate calculation when:
   * 1. The current position changes (new FEN)
   * 2. Both engines (Stockfish and Maia) are ready
   * 3. No calculation is currently in progress
   *
   * A 300ms debounce prevents rapid recalculation during fast navigation
   * through moves (e.g., arrow key navigation through game history).
   */
  const autoCalculationTimerRef = useRef<NodeJS.Timeout | null>(null)
  const lastCalculatedFenRef = useRef<string | null>(null)

  useEffect(() => {
    // Clear any pending auto-calculation when position changes
    if (autoCalculationTimerRef.current) {
      clearTimeout(autoCalculationTimerRef.current)
      autoCalculationTimerRef.current = null
    }

    // Only auto-calculate if:
    // - We have a valid position
    // - Engines are ready
    // - Not already calculating
    // - Position is different from last calculated
    if (
      currentNode?.fen &&
      isEnginesReady &&
      !progress.isCalculating &&
      currentNode.fen !== lastCalculatedFenRef.current
    ) {
      // Debounce to prevent rapid recalculation during fast navigation
      autoCalculationTimerRef.current = setTimeout(() => {
        lastCalculatedFenRef.current = currentNode.fen
        startCalculation()
      }, 300)
    }

    // Cleanup timer on unmount or position change
    return () => {
      if (autoCalculationTimerRef.current) {
        clearTimeout(autoCalculationTimerRef.current)
      }
