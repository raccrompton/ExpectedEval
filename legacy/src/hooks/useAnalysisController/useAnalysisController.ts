...
import { useExpectedWinrateController } from '../useExpectedWinrateController'
...
export const useAnalysisController = (
  ...
  const saveAnalysisToBackend = useCallback(async () => {
    ...
    try {
      ...
      console.log('Analysis processed:', analysisData.length, 'positions')
    } catch (error) {
  ...
  // Expected Winrate controller - provides probability tree analysis
  const expectedWinrate = useExpectedWinrateController(
    controller.currentNode || null,
    inProgressAnalyses,
    {
      enableAutoSave,
  ...
  return {
    ...
    // Expected Winrate analysis - provides probability-weighted winrate calculations
    expectedWinrate,
  }
}
