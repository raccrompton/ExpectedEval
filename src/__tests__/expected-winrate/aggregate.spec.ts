import { computeCoverage, computeExpected, averageDepth, confidenceFromCoverage } from '../../lib/expectedWinrate/aggregate'
import { ExpectedWinrateNode } from '../../lib/expectedWinrate/types'

describe('aggregate utils', () => {
  const nodes: ExpectedWinrateNode[] = [
    { fen: 'f1', path: ['e2e4'], cumulativeProb: 0.6, turn: 'b', pruned: false, leaf: { winrate: 0.7, depth: 20 } },
    { fen: 'f2', path: ['d2d4'], cumulativeProb: 0.2, turn: 'b', pruned: false, leaf: { winrate: 0.3, depth: 20 } },
  ]
