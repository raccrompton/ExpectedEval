...
export const OpeningSelectionModal: React.FC<Props> = ({
  ...
  // Helper function to create initial drill sequence (just the first drill)
  const createInitialDrillSequence = (
    selections: OpeningSelection[],
  ): OpeningSelection[] => {
    if (selections.length === 0) return []

    // Create a copy of selections and shuffle them
    const shuffled = [...selections].sort(() => Math.random() - 0.5)

    // Return the first drill with a unique ID
    const timestamp = Date.now()
    return [
      {
        ...shuffled[0],
        id: `${shuffled[0].id}-${timestamp}-0`,
      },
    ]
  }
  ...
  const handleStartDrilling = async () => {
    if (selections.length > 0) {
      const drillSequence = createInitialDrillSequence(selections)
